...
async function patchEntryServer(port: number): Promise<void> {
  ...
  if (original.includes('initAuth') && original.includes('_anby_token')) {
    console.log(
      kleur.dim('• app/entry.server.tsx already has initAuth + token redirect — no patch needed'),
    );
    return;
  }
  ...
  if (original.includes('initAuth') && !original.includes('_anby_token')) {
    ...
    if (!original.includes('handleRequest')) {
      console.log(
        kleur.dim('• app/entry.server.tsx has initAuth but no handleRequest — skipping token redirect patch'),
      );
      return;
    }
...
`;
      ...
      console.log(
        kleur.dim('• Could not locate handleRequest body — add _anby_token handling manually'),
      );
    }
    return;
  }
